...
const ApplicationReviewModal = ({ studentId, onClose, onRefresh }) => {
    ...
    const fetchDetails = async () => {
        ...
        try {
            ...
            const [studentRes, adminRes] = await Promise.all([
                axios.get(`http://127.0.0.1:8000/api/students/${studentId}`, { headers }),
                axios.get(`http://127.0.0.1:8000/api/admin/students/${studentId}`, { headers }).catch(() => null)
            ]);
            
            const student = studentRes.data.data;
            
            const pStatus = adminRes?.data?.data?.paymentStatus || 'Due';
    ...
    const handleApproveApplication = async () => {
        ...
        try {
            const token = localStorage.getItem('access_token');
            await axios.post(`http://127.0.0.1:8000/api/students/${studentId}/approve`, {
                package_id: selectedPackage,
            }, { headers: { Authorization: `Bearer ${token}` } });
            
            showNotification('success', "Application approved! 50% Deposit Invoice sent to student.");
            setTimeout(() => {
                if (onRefresh) onRefresh();
                onClose();
            }, 1500);
        } catch (error) {
            showNotification('error', error.response?.data?.error || "Approval failed");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleAssignInstructor = async () => {
        ...
        try {
            const token = localStorage.getItem('access_token');
            await axios.post(`http://127.0.0.1:8000/api/students/${studentId}/assign-instructor`, {
                instructor_id: selectedInstructor
            }, { headers: { Authorization: `Bearer ${token}` } });
            
            showNotification('success', "Instructor assigned and student fully activated!");
            setTimeout(() => {
                if (onRefresh) onRefresh();
                onClose();
            }, 1500);
        } catch (error) {
            showNotification('error', error.response?.data?.error || "Assignment failed");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleRejectSubmit = async (reason) => {
        ...
        try {
            const token = localStorage.getItem('access_token');
            await axios.post(`http://127.0.0.1:8000/api/students/${studentId}/reject`, 
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-50 p-2 sm:p-4 transition-all">
            <div className="relative bg-white dark:bg-slate-950 rounded-2xl shadow-2xl w-full max-w-7xl max-h-[95vh] overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800">
                
                {/* INLINE NOTIFICATION BANNER */}
                {notification && (
                    <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-4 fade-in duration-300 ${
                        ...
                        <div className="space-y-12">
                            ...
                            <div className="bg-slate-50 dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 mt-10 shadow-inner">
                                <h3 className="text-base sm:text-lg font-bold mb-6 flex items-center gap-3 text-slate-800 dark:text-slate-200">
                                    ...
                                    {data.user?.status === 'awaiting_payment' ? 'Step 2: Assign Instructor' : 'Step 1: Approve & Invoice'}
                                </h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
                                    <div className="space-y-2">
                                        ...
                                        <select 
                                            className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-3 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 dark:text-slate-200 transition-all shadow-sm disabled:opacity-50"
                                            value={selectedPackage}
                                            onChange={(e) => setSelectedPackage(e.target.value)}
                                            disabled={data.user?.status === 'awaiting_payment'} 
                                        >
                                    ...
                                    {data.user?.status === 'awaiting_payment' && (
                                        <div className="space-y-2">
                                ...
                                {data.user?.status === 'pending' ? (
                                    <button 
                                        ...
                                        className="w-full mt-8 py-4 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold text-sm sm:text-base transition-all disabled:opacity-40 shadow-lg shadow-teal-500/20"
                                    >
                                        {submitting ? "Processing..." : "Approve Application & Send 50% Deposit Invoice"}
                                    </button>
                                ) : data.user?.status === 'awaiting_payment' ? (
                                    // DISABLED BASED ON PAYMENT STATE
                                    <button 
                                        onClick={handleAssignInstructor}
                                        disabled={submitting || !selectedInstructor || paymentStatus === 'Awaiting Payment' || paymentStatus === 'Due'}
                                        className="w-full mt-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm sm:text-base transition-all disabled:opacity-40 shadow-lg shadow-emerald-500/20 flex flex-col items-center justify-center gap-1"
                                    >
                                        <span>{submitting ? "Processing..." : "Verify Payment & Fully Activate Student"}</span>
                                        {(paymentStatus === 'Awaiting Payment' || paymentStatus === 'Due') && (
                                            <span className="text-xs font-medium text-emerald-200">
                                                (Disabled: 50% Deposit Not Yet Recorded)
                                            </span>
                                        )}
                                    </button>
                                ) : null}
                                ...
                                <button 
                                    ...
                                    className="w-full mt-4 py-3 bg-white dark:bg-slate-900 border-2 border-red-100 dark:border-red-900/30 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 dark:text-red-500 rounded-xl font-bold text-sm sm:text-base transition-all"
                                >
                                    Reject Application
                                </button>
                            </div>
