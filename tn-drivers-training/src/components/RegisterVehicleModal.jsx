      ...
      <div className="bg-white dark:bg-[#111827] w-full max-w-xl rounded-t-[2.5rem] md:rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden animate-in slide-in-from-bottom md:zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto scrollbar-hide">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <Gauge size={14} className="text-[#008B8B]"/> odometer (KM)
              </label>
              <input required type="number" className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-4 py-3.5 text-sm font-bold dark:text-white focus:ring-2 focus:ring-[#008B8B]/20 outline-none" placeholder="12500" value={formData.odometerKm} onChange={(e) => setFormData({...formData, odometerKm: e.target.value})} />
            </div>
          </div>s

          {/* Section 3: Compliance Info (NEW FIELDS ADDED HERE) */}
          <div className="bg-slate-50 dark:bg-slate-800/40 p-5 rounded-[2rem] border border-slate-100 dark:border-slate-800 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <ShieldCheck size={14} className="text-orange-500"/> Insurance No.
                </label>
                <input required className="w-full bg-white dark:bg-slate-900 border-none rounded-xl px-4 py-2.5 text-xs font-bold dark:text-white outline-none focus:ring-2 focus:ring-orange-500/20" placeholder="INS-99234" value={formData.insuranceNo} onChange={(e) => setFormData({...formData, insuranceNo: e.target.value})} />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Bookmark size={14} className="text-blue-500"/> RC Number
                </label>
                <input required className="w-full bg-white dark:bg-slate-900 border-none rounded-xl px-4 py-2.5 text-xs font-bold dark:text-white outline-none focus:ring-2 focus:ring-blue-500/20" placeholder="RC-1102-NL" value={formData.rcNo} onChange={(e) => setFormData({...formData, rcNo: e.target.value})} />
              </div>
            </div>
